...
phoneInput.addEventListener('input', function () {
  ...
  // Utiliza una expresión regular para validar si es un número
  const isNumber = /^\d+$/.test(inputValue);

  // Muestra o oculta el mensaje de error según la validación
  if (isNumber) {
    phoneError.textContent = '';
    submitBtn.removeAttribute('disabled'); // Si es un número, no hay error
  } else {
    phoneError.textContent = 'Por favor, introduce un número válido';
    submitBtn.setAttribute('disabled', 'true'); // Si no es un número, muestra el mensaje de error
  }
});

function watchLinks(event) {
...
images.forEach(function (img) {
  img.onclick = function () {
    ...
    modalParagraph.textContent = document.getElementById(
      `${this.id}Pg`
    ).textContent;
  };
});
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.getElementById('astronauts').play();
